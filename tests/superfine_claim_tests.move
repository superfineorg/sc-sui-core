...
module superfine::superfine_claim_tests {
	...
	use sui::object::ID;
	use sui::address;
	use superfine::superfine_claim::{Self, ClaimingVault};
	...
	public fun test_claim_assets() {
		...
		{
			let vault_value = test_scenario::take_shared<ClaimingVault>(scenario);
			let vault = &mut vault_value;
			let ctx = test_scenario::ctx(scenario);
			superfine_claim::set_operator(vault, operator, true, ctx);
			test_scenario::return_shared(vault_value);
		};
		...
		test_scenario::next_tx(scenario, campaign_creator);
		{
			...
			let ctx = test_scenario::ctx(scenario);
			listing_id = superfine_claim::list_asset<ExampleNft>(vault, nft, ctx);
			test_scenario::return_shared(vault_value);
		};
		...
		test_scenario::next_tx(scenario, campaign_creator);
		{
			...
			let vault = &mut vault_value;
			let ctx = test_scenario::ctx(scenario);
			superfine_claim::delist_asset<ExampleNft>(vault, listing_id, ctx);
			test_scenario::return_shared(vault_value);
		};
		...
		test_scenario::next_tx(scenario, campaign_creator);
		{
			...
			let nft = test_scenario::take_from_sender<ExampleNft>(scenario);
			let ctx = test_scenario::ctx(scenario);
			listing_id = superfine_claim::list_asset(vault, nft, ctx);
			test_scenario::return_shared(vault_value);
		};
